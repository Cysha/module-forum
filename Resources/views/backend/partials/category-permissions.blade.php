        ...
        <div class="row permission-row">
            ...
            <div class="col-md-7">
                ...
                @if (empty($resource_id) || $resource_id === 0)
                    @set($permissionStr, $permission->action.'@'.$permission->resource_type)
                @else
                    @set($permissionStr, $permission->action.'@'.$permission->resource_type.':'.$resource_id)
                @endif

                <select name="permissions[{{ $role->id }}][{{ $permissionStr }}]" class="form-control">
...
@else
    <div class="alert alert-info">Info: No permissions found.</div>
@endif
</div>
